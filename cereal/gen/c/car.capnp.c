...
void cereal_read_CarState(struct cereal_CarState *s, cereal_CarState_ptr p) {
	...
	s->steeringTorqueEps = capn_to_f32(capn_read32(p.p, 40));
	...
	s->clutchPressed = (capn_read8(p.p, 9) & 8) != 0;
...
void cereal_write_CarState(const struct cereal_CarState *s, cereal_CarState_ptr p) {
	...
	capn_write32(p.p, 40, capn_from_f32(s->steeringTorqueEps));
	...
	capn_write1(p.p, 75, s->clutchPressed != 0);
...
void cereal_read_CarParams(struct cereal_CarParams *s, cereal_CarParams_ptr p) {
	...
	s->safetyModelPassive = (enum cereal_CarParams_SafetyModel)(int) capn_read16(p.p, 62);
	...
	case cereal_CarParams_lateralTuning_lqr:
		s->lateralTuning.lqr.p = capn_getp(p.p, 9, 0);
		break;
	default:
		break;
	}
	s->steerLimitAlert = (capn_read8(p.p, 0) & 32) != 0;
	s->steerLimitTimer = capn_to_f32(capn_read32(p.p, 76));
	...
	s->isPandaBlack = (capn_read8(p.p, 1) & 4) != 0;
	s->dashcamOnly = (capn_read8(p.p, 1) & 8) != 0;
	s->transmissionType = (enum cereal_CarParams_TransmissionType)(int) capn_read16(p.p, 68);
	s->carFw.p = capn_getp(p.p, 11, 0);
	s->radarTimeStep = capn_to_f32(capn_read32(p.p, 72) ^ 0x3d4ccccdu);
	s->communityFeature = (capn_read8(p.p, 1) & 16) != 0;
}
void cereal_write_CarParams(const struct cereal_CarParams *s, cereal_CarParams_ptr p) {
	...
	capn_write16(p.p, 62, (uint16_t) (s->safetyModelPassive));
	...
	case cereal_CarParams_lateralTuning_indi:
	case cereal_CarParams_lateralTuning_lqr:
		capn_setp(p.p, 9, s->lateralTuning.lqr.p);
		break;
	default:
		break;
	}
	capn_write1(p.p, 5, s->steerLimitAlert != 0);
	capn_write32(p.p, 76, capn_from_f32(s->steerLimitTimer));
	...
	capn_write1(p.p, 10, s->isPandaBlack != 0);
	capn_write1(p.p, 11, s->dashcamOnly != 0);
	capn_write16(p.p, 68, (uint16_t) (s->transmissionType));
	capn_setp(p.p, 11, s->carFw.p);
	capn_write32(p.p, 72, capn_from_f32(s->radarTimeStep) ^ 0x3d4ccccdu);
	capn_write1(p.p, 12, s->communityFeature != 0);
}
...
cereal_CarParams_LateralLQRTuning_ptr cereal_new_CarParams_LateralLQRTuning(struct capn_segment *s) {
	cereal_CarParams_LateralLQRTuning_ptr p;
	p.p = capn_new_struct(s, 16, 5);
	return p;
}
cereal_CarParams_LateralLQRTuning_list cereal_new_CarParams_LateralLQRTuning_list(struct capn_segment *s, int len) {
	cereal_CarParams_LateralLQRTuning_list p;
	p.p = capn_new_list(s, len, 16, 5);
	return p;
}
void cereal_read_CarParams_LateralLQRTuning(struct cereal_CarParams_LateralLQRTuning *s, cereal_CarParams_LateralLQRTuning_ptr p) {
	capn_resolve(&p.p);
	s->scale = capn_to_f32(capn_read32(p.p, 0));
	s->ki = capn_to_f32(capn_read32(p.p, 4));
	s->dcGain = capn_to_f32(capn_read32(p.p, 8));
	s->a.p = capn_getp(p.p, 0, 0);
	s->b.p = capn_getp(p.p, 1, 0);
	s->c.p = capn_getp(p.p, 2, 0);
	s->k.p = capn_getp(p.p, 3, 0);
	s->l.p = capn_getp(p.p, 4, 0);
}
void cereal_write_CarParams_LateralLQRTuning(const struct cereal_CarParams_LateralLQRTuning *s, cereal_CarParams_LateralLQRTuning_ptr p) {
	capn_resolve(&p.p);
	capn_write32(p.p, 0, capn_from_f32(s->scale));
	capn_write32(p.p, 4, capn_from_f32(s->ki));
	capn_write32(p.p, 8, capn_from_f32(s->dcGain));
	capn_setp(p.p, 0, s->a.p);
	capn_setp(p.p, 1, s->b.p);
	capn_setp(p.p, 2, s->c.p);
	capn_setp(p.p, 3, s->k.p);
	capn_setp(p.p, 4, s->l.p);
}
void cereal_get_CarParams_LateralLQRTuning(struct cereal_CarParams_LateralLQRTuning *s, cereal_CarParams_LateralLQRTuning_list l, int i) {
	cereal_CarParams_LateralLQRTuning_ptr p;
	p.p = capn_getp(l.p, i, 0);
	cereal_read_CarParams_LateralLQRTuning(s, p);
}
void cereal_set_CarParams_LateralLQRTuning(const struct cereal_CarParams_LateralLQRTuning *s, cereal_CarParams_LateralLQRTuning_list l, int i) {
	cereal_CarParams_LateralLQRTuning_ptr p;
	p.p = capn_getp(l.p, i, 0);
	cereal_write_CarParams_LateralLQRTuning(s, p);
}

cereal_CarParams_CarFw_ptr cereal_new_CarParams_CarFw(struct capn_segment *s) {
	cereal_CarParams_CarFw_ptr p;
	p.p = capn_new_struct(s, 8, 1);
	return p;
}
cereal_CarParams_CarFw_list cereal_new_CarParams_CarFw_list(struct capn_segment *s, int len) {
	cereal_CarParams_CarFw_list p;
	p.p = capn_new_list(s, len, 8, 1);
	return p;
}
void cereal_read_CarParams_CarFw(struct cereal_CarParams_CarFw *s, cereal_CarParams_CarFw_ptr p) {
	capn_resolve(&p.p);
	s->ecu = (enum cereal_CarParams_Ecu)(int) capn_read16(p.p, 0);
	s->fwVersion = capn_get_text(p.p, 0, capn_val0);
}
void cereal_write_CarParams_CarFw(const struct cereal_CarParams_CarFw *s, cereal_CarParams_CarFw_ptr p) {
	capn_resolve(&p.p);
	capn_write16(p.p, 0, (uint16_t) (s->ecu));
	capn_set_text(p.p, 0, s->fwVersion);
}
void cereal_get_CarParams_CarFw(struct cereal_CarParams_CarFw *s, cereal_CarParams_CarFw_list l, int i) {
	cereal_CarParams_CarFw_ptr p;
	p.p = capn_getp(l.p, i, 0);
	cereal_read_CarParams_CarFw(s, p);
}
void cereal_set_CarParams_CarFw(const struct cereal_CarParams_CarFw *s, cereal_CarParams_CarFw_list l, int i) {
	cereal_CarParams_CarFw_ptr p;
	p.p = capn_getp(l.p, i, 0);
	cereal_write_CarParams_CarFw(s, p);
}
